...
| Render Structured copy pipeline | Complete (repository) | `715d8bc`, `d8d75b7`, `7f8d525` | 14 focused Render generation/delivery/diagnostic tests, 188 full regression tests, standalone smoke assertions, backend/frontend compilation, lints, and Graphify update pass | Runs provider calls and prompt assembly on Render without a running local agent; normalizes dashboard model IDs for provider APIs; retains only each user's five most recent sanitized provider diagnostics (never keys, headers, prompt bodies, or generated response bodies); stores encrypted TTL prompt delivery ciphertext; imports final prompts locally before acknowledgement and deletion |
