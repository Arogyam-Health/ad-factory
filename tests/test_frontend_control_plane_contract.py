...
class FrontendControlPlaneContractTests(unittest.TestCase):
    ...
    def test_structured_copy_runs_on_render_and_reference_hydration_stays_local(self) -> None:
        ...
        self.assertIn("Browser automation", studio)
        self.assertIn("adFactoryCopyBrowserEngine", studio)
        self.assertIn('provider === "browser"', pipeline)
        self.assertIn("copyBrowserEngine", pipeline)
        ...
        self.assertIn("localReferenceOwner", reference)
        self.assertIn("orgId !== \"personal\"", reference)
        self.assertNotIn('ownerType: "user"', reference)
        self.assertNotIn("[props.deviceId, props.setStatus]", reference)
        self.assertIn("localDataPlane.session(props.deviceId)", reference)
        self.assertIn("last?.text === next", studio)
    ...
    def test_guide_page_and_flexible_formats_are_wired(self) -> None:
        ...
        self.assertIn('path="/docs/:docName"', app)
        self.assertIn('path="/STRUCTURED_COPY_SYSTEM.md"', app)
        ...
        self.assertIn("Global formats", studio)
        self.assertIn("applies to every selected card", studio)
        self.assertIn("usedPatternFormats", studio)
        self.assertIn("persona-formats", studio)
        self.assertIn("PatternSelect", studio)
        self.assertNotIn("persona-patterns", studio)
        self.assertNotIn("visual_archetypes_by_persona", studio)
        self.assertIn("visual_archetypes_by_format", studio)
        self.assertIn("stopPropagation", studio)
        self.assertIn("formats_by_persona", studio)
        self.assertIn("togglePersonaFormat", studio)
        self.assertIn('to="/guide"', config)
        self.assertIn("result.notice", config)
        self.assertIn("ConfigFileEditor", config)
        self.assertIn("config-page", config)
        self.assertIn("Copy to my config", config)
        self.assertIn("copy-to-personal", config)
        self.assertIn("summarizeConfigValue", studio)
        self.assertIn("/api/guide", guide)
        self.assertIn("guide-toc", guide)
        self.assertIn("guide-desk", guide)
        self.assertIn("How to use the editor", doc)
        self.assertIn("Copy to my config", doc)
        self.assertIn("Field", doc)
        self.assertIn("https://github.com/Vinay-003/ad-factory/tree/render-setup/docs", doc)
        self.assertIn("DEVELOPER_CLOUD_MIGRATION.md", doc)
        self.assertIn("Which file does what", doc)
        self.assertIn("github.com/Vinay-003/ad-factory/tree/render-setup", guide)
        self.assertIn("guide-toc-group", guide)
        self.assertIn("modal-resize", _read("components", "Modal.tsx"))
        self.assertIn('"/api/guide"', defaults)
        self.assertIn('"/api/docs/{name}"', defaults)
        control = (ROOT / "dashboard" / "backend" / "control_app.py").read_text(encoding="utf-8")
        app_py = (ROOT / "dashboard" / "backend" / "app.py").read_text(encoding="utf-8")
        self.assertIn('"/api/guide"', control)
        self.assertIn('"/api/docs/"', control)
        self.assertIn('"/api/guide"', app_py)
        self.assertIn('"/api/docs/"', app_py)
        local_readme = (ROOT / "docs" / "LOCAL_AGENT_README.md").read_text(encoding="utf-8")
        self.assertIn("Python 3.12 exactly", local_readme)
        self.assertIn("cgi", local_readme)
        self.assertIn("STRUCTURED_COPY_SYSTEM.md", defaults)
        ...
        cloud = (ROOT / "docs" / "DEVELOPER_CLOUD_MIGRATION.md").read_text(encoding="utf-8")
        self.assertIn("STORAGE_PROVIDER", cloud)
        self.assertIn("Amazon S3", cloud)
        self.assertIn("/api/runs/{run_id}/image-generation", cloud)
    ...
    def test_studio_org_and_copy_pipeline_persist_across_reload(self) -> None:
        ...
        self.assertIn("overlayLocalRunStats", studio)
        self.assertIn("overlayLocalRunStats", _read("lib", "run-flow.ts"))
        self.assertIn("removeItem(\"adFactoryCopyPipeline\")", studio)
        self.assertIn("deleteLocalRuns", studio)
        self.assertIn("dropRunsFromUi", studio)
        self.assertNotIn("if (remembered.length) next = mergeRunLists", studio)
        self.assertNotIn("display_batch: openRunId", studio)
        ...
        self.assertIn("<OrgConfigChips", studio.split('kicker="03 · Copy desk"')[0])
        self.assertNotIn("<OrgConfigChips", studio.split('kicker="03 · Copy desk"')[1])
        self.assertIn("compose-toolbar", studio)
        self.assertIn("compose-toolbar-configs", studio)
        self.assertIn("compose-toolbar-langs", studio)
        self.assertIn("compose-body", studio)
        ...
        self.assertIn("compose-toolbar", dist_css)
        self.assertIn("Show local images", dist_js)
        ...
        self.assertIn("PressDrawer", reference)
        self.assertIn("press-stack", reference)
        self.assertIn("press-drawer", _read("components", "Tile.tsx"))
        self.assertIn("persona-card-head", reference)
        self.assertIn("overflow-y: auto", _read("styles", "global.css"))
        self.assertIn("className=\"masthead\"", _read("components", "Shell.tsx"))
        self.assertIn("attachScrollChain", _read("components", "Shell.tsx"))
        self.assertIn("atScrollEnd", _read("lib", "scroll-chain.ts"))
        self.assertIn("studio-row", studio)
        self.assertIn("PressDrawer", studio)
        self.assertIn("press-stack", studio)
        self.assertIn("asset-card-open", studio)
        self.assertIn('window.open(url, "_blank", "noopener")', studio)
        self.assertIn("run-row-error", studio)
        self.assertIn(".run-row-error", _read("styles", "global.css"))
        self.assertIn("grid-column: span 6", _read("styles", "global.css"))
        self.assertIn("press-cols", _read("components", "Tile.tsx"))
        self.assertIn("press-drawer-wide", _read("components", "Tile.tsx"))
        self.assertIn('<PressDrawer title="Batch" wide>', studio)
        self.assertIn("minmax(0, 1fr) minmax(0, 1fr)", _read("styles", "global.css"))
        self.assertIn("@media (max-width: 1180px)", _read("styles", "global.css"))
        self.assertIn("adFactoryStudioSession", _read("lib", "studio-session.ts"))
        self.assertIn("structuredLogLines", _read("lib", "studio-session.ts"))
        self.assertIn("referenceLogLines", _read("lib", "studio-session.ts"))
        self.assertIn("logLinesByFlow", studio)
        self.assertIn("appendStructuredLog", studio)
        self.assertIn("appendReferenceLog", studio)
        self.assertIn("readStudioSession", studio)
        self.assertIn("adFactoryStudioBatch", _read("lib", "studio-batch.ts"))
        self.assertIn("readStudioBatch", studio)
        self.assertIn("writeStudioBatch", studio)
        self.assertNotIn("if (defaults.batch_size) setBatchSize", studio)
        self.assertIn("reusableRuns", studio)
        self.assertIn("share_background_across_personas: shareBg", studio)
        self.assertIn("reuse_backgrounds_from_run_id: reuseBg", studio)
        self.assertIn("reuse_visual_patterns_from_run_id: reusePattern", studio)
        self.assertIn("batch_size: batchSize", studio)
        ...
        self.assertIn("[runId, deviceId, agentId, run.device_id, paired, busy, reloadToken, refreshToken]", workspace)
        self.assertIn("listRuns(deviceId)", studio)
        self.assertNotIn("listRunsAcrossOwners", studio)
        self.assertIn("prev.structured !== activeRunId", studio)
        self.assertNotIn('setFlowOpenRun("structured", activeRunId)', studio)
        self.assertIn("retry: 2", reference)
        self.assertIn("<!DOCTYPE", api)
        self.assertIn("Retry reference generation if this plate is empty", workspace)
        self.assertIn("Math.max(Number(extra.prompt_count", _read("lib", "run-flow.ts"))
        self.assertNotIn("storedOwnerKeys", workspace)
        self.assertIn("localDevice && paired", workspace)
        self.assertIn("Show local images", workspace)
        self.assertIn("deskPromptStatus", workspace)
        self.assertIn("available_local", workspace)
        self.assertNotIn("plateOwner", studio)
        self.assertNotIn("localOwner", workspace)
        self.assertIn("promptContent(id, device", workspace)
        self.assertIn("await loadFlowRuns(flow, true)", studio)
        ...
        self.assertIn("IMAGES_PER_PAGE = 8", gallery)
        self.assertIn("fetchFails", gallery)
        self.assertIn("ListPager", gallery)
        ...
        self.assertIn("PROMPTS_PER_PAGE = 8", workspace)
        self.assertIn("ListPager", workspace)
        self.assertNotIn("slice(0, 16)", workspace)
        self.assertNotIn("slice(0, 40)", workspace)
        self.assertIn("noCache: true", workspace)
        ...
        self.assertIn("inferLogLevel", studio)
        self.assertGreater(studio.rfind("<RunTerminal"), studio.find('<PressDrawer title="Run"'))
        self.assertGreater(studio.rfind("<RunTerminal"), studio.find("<ReferenceDesk"))
    ...
    def test_studio_init_does_not_block_on_local_assets_and_pages_show_skeletons(self) -> None:
        ...
        self.assertIn("_repairSession", _read("lib", "local-data-plane.js"))
